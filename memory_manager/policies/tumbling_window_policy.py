...
class TumblingWindowPolicy(BasePolicy):
    """
    Memory management policy implementing a tumbling window.
    Maintains a batch of observable units up to a fixed window size, then starts a new batch.
    """

    @override
    def __init__(self, window_size: int) -> None:
        """
        Initialize the TumblingWindowPolicy.
        Args:
            window_size (int): The maximum number of units in each window.
        """
        super().__init__()
    ...
    def update(self, unit: BaseObservableUnit) -> None:
        ...
        Adds the unit to the current window. If the window exceeds its size, starts a new window with the latest unit.
        Args:
